...
export function ScoreGridHeader() {
  return (
    <div className={clsx(GRID_COLS, 'bg-slate-50')}>
      <span className="px-3 py-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Criterion</span>
      {SCORES.map((n) => (
        <span key={n} className="flex items-center justify-center py-2 text-sm font-semibold text-slate-500">
          {n}
        </span>
      ))}
    </div>
...
export function ScoreGridRow({ label, value, onChange }: ScoreGridRowProps) {
  return (
    <div className={GRID_COLS} role="radiogroup" aria-label={label}>
      <p className="flex items-center px-3 py-2 text-xs font-medium text-slate-700">{label}</p>
      {SCORES.map((n) => {
        ...
        return (
          <div key={n} className="flex items-center justify-center py-2">
            <button
              ...
              onClick={() => onChange(n)}
              className={clsx(
                'flex h-6 w-6 items-center justify-center rounded transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2',
                checked
