...
Plain ESM JavaScript, no build step, no TypeScript, no ORM — see `server/db.js` (opens the SQLite connection and applies `server/schema.sql` on startup), `server/routes/*.js` (one router per resource: `auth`, `candidates`, `interviews`, `users`), and `server/middleware/auth.js` (`requireAuth`/`requireRole` guard using opaque session tokens stored in the `sessions` table, not JWTs). Config values read from env use `||` (not `??`) so a blank `KEY=` line copied from `.env.example` falls back to the default — a blank `DB_PATH` would otherwise open a throwaway SQLite database. Because `better-sqlite3` and `bcryptjs`'s sync API are used throughout, most route handlers are synchronous — no `async`/`await` needed. The exceptions are handlers that send email via `server/lib/mailer.js` (nodemailer), which is inherently async — see `POST /api/interviews/:id/notify` in `server/routes/interviews.js`.
